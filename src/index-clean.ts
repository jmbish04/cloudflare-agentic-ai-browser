...
app.get("/progress", async (c) => {
  ...
  if (!jobId || isNaN(parseInt(jobId, 10))) {
    return c.html(`
      <p>No valid job ID provided. <a href="/">Return to dashboard</a></p>
    `, 400);
  }
...
export class Browser {
  ...
  async fetch(request: Request) {
    ...
    // Legacy streaming path - restore backwards compatibility
    ...
    return new Response("Invalid request. Missing baseUrl or goal.", { status: 400 });
  }
  ...
  private async executeJob(jobId: number, baseUrl: string, goal: string) {
    ...
    try {
      ...
      do {
        ...
        for (const toolCall of toolCalls) {
          ...
          try {
            ...
            // Wait for UI to update - pragmatic solution for dynamic sites
      ...
      // Include error details in job log for user visibility
  ...
  private async storeScreenshot(page: puppeteer.Page, folder: string, suffix?: string) {
    const fileName = "screenshot_" + new Date().toISOString() + (suffix ? `_${suffix}` : "");
