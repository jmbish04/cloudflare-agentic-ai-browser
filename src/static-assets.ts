...
async function submitJob(event) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Open progress page with the specific job ID to avoid race condition
...
async function checkForSpecificJob(jobId) {
  try {
    ...
    if (response.ok) {
      ...
      // If job is found, redirect to its detail page
    ...
    if (checkCount < MAX_CHECKS) {
      setTimeout(() => checkForSpecificJob(jobId), POLLING_INTERVAL_MS);
    } else {
...
// Extract jobId from URL parameters and start tracking specific job
const urlParams = new URLSearchParams(window.location.search);
const jobId = urlParams.get('jobId');

if (jobId) {
  // Start checking for the specific job after a short delay
  setTimeout(() => checkForSpecificJob(jobId), 2000);
} else {
